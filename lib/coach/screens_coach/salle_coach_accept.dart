...
class SalleCoachAccept extends StatefulWidget {
  const SalleCoachAccept({Key? key}) : super(key: key);

  @override
  _SportifCoteResponsableState createState() => _SportifCoteResponsableState();
}

class _SportifCoteResponsableState extends State<SalleCoachAccept> {
  ...
  Future<List<ExerciceObject>> getData() async{
    ...
    QuerySnapshot<Map<String,dynamic>> data = await firestore.collection("Members").where('userID',isEqualTo: uid).where("role",isEqualTo: "coach").get();
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      floatingActionButton: FloatingActionButton(
        ...
        onPressed: (){
          Navigator.of(context).push(CupertinoPageRoute(builder: (context)=>DemandeSalleCoach()));
        },
      ),
    );
  }
